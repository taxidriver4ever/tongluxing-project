-- 同路行 Mock 联调数据：用户、车辆、认证、公开行程和推荐关系
-- MySQL 8.0+
--
-- 执行顺序：
--   1. 01_reset_and_create_all_tables.sql
--   2. 02_mock_users_and_trips.sql
--
...
-- 使用第一条 Mock 行程作为稳定推荐源；目标行程写入后再参与匹配结果生成。
SET @source_trip_id := 920000000000010201;

-- 六条公开招募行程，出发时间以执行时刻为基准，避免演示数据过期。
INSERT INTO trip (
    id, trip_number, user_id, vehicle_id, title, description, expected_people,
